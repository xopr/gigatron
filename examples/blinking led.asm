...
; ------------
    ...
    ld #2, x                ; Operand on bus, bus to X (X = 2)
    st #0b1010, x           ; Operand on bus, store bus at RAM[X]
...
blink:
    ...
    ld #2, x                ; X = 2
    ld x                    ; load RAM[X]
    xora #0x0f              ; invert the lower nibble (4 bits)
    st x                    ; Store AC in RAM[X]
    ld x                    ; Load RAM[X] on bus
    ...
    ; The inner loops are configured to delay _approximately_ 10ms
    ; 2pqr+6qr+7r
    ; Run the outer loop 100 times (=1s)
    ld #0, x                ; X = 0
    st #100, x              ; Store 100 in RAM[X]
loop1:
    ld #1, x                ; X = 1
    st #120, x              ; Store 120 in RAM[X]
loop2:

    LD #255                 ; AC = 255
loop3:
    ; inner loop, 2 instructions per cycle
    BNE #loop3              ; If AC != 0, jump back to #loop3
    SUBA #1                 ; AC = AC - 1 (still part of loop3)

    ; mid loop, 1 + p + 5 instructions per cycle
    ld #1, x                ; X = 1
    ld x                    ; AC = RAM[X]
    suba #1                 ; AC = AC - 1
    bne #loop2              ; If AC != 0, jump back to #loop2
    st x                    ; RAM[X] = AC  (still part of loop2)
    ; outer loop, 2 + q + 5 instructions per cycle
    ld #0, x                ; X = 0
    ld x                    ; AC = RAM[X]
    suba #1                 ; AC = AC - 1
    bne #loop1              ; If AC != 0, jump back to #loop2
    st x                    ; RAM[X] = AC  (still part of loop2)
